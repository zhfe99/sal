function wsBk = movBk(src, parBk, varargin)
...
if svL == 2 && exist(matpath, 'file')
    prInOut('movBk', 'old, %s', prex);
    wsBk = matFld(matpath, 'wsBk');
    wsBk.vdo = vdopath;
    return;
end
prIn('movBk', 'new, nm %s, wMa %d, wF %d, dire %s, th %d', src.tag, wMa, wF, dire, th);
hTi = tic;
