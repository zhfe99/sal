function wsSal = movSalCb(src, wsSmp, wsFlow, wsAcc, wsBk, parSal, varargin)
% Combine video salience by combining different sources.
%
% Input
%   src      -  source
%   wsSmp    -  sampling
...
if svL == 2 && exist(matpath, 'file')
    prInOut('movSalCb', 'old, %s', prex);
    wsSal = matFld(matpath, 'wsSal');
    wsSal.vdo = vdopath;
    return;
end
prIn('movSalCb', 'new, %s', prex);
