function wsSal = movSalImg(src, parSal, varargin)
...
fold = sprintf('mov/%s/salImg', src.tag);
...
if svL == 2 && exist(matpath, 'file')
    prInOut('movSalImg', 'old, %s', prex);
    wsSal = matFld(matpath, 'wsSal');
    wsSal.vdo = vdopath;
    return;
end
prIn('movSalImg', 'new, %s', prex);
hTi = tic;
...
elseif strcmp(alg, 'rahtu')
    ...
    %% flow
    wsFlow0 = movFlowLiu(src, [], 'svL', 2);
    hrFlow = movRIn(src, 'flow', []);
    Vs = cell(1, nF);
    for iF = 1 : nF
        mat = vdoR(hrFlow, iF);
