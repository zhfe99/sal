function wsSeg = movSegL(src, wsVox, parSeg, varargin)
...
fold = sprintf('mov/%s/seg', src.tag);
...
if svL == 2 && exist(matpath, 'file')
    prInOut('movSegL', 'old, %s', prex);
    wsSeg = matFld(matpath, 'wsSeg');
    wsSeg.vdo = vdopath;
    return;
end
prIn('movSegL', 'new, %s', prex);
hTi = tic;

% streaming voxel -> global label
wsSeg0 = movSegLM2L(src, wsVox, 'svL', 2);
[L0s, mSeg0, A0, keys] = stFld(wsSeg0, 'Ls', 'mSeg', 'A', 'keys');

% merge
wsSeg = movSegLMerge(src, wsSeg0, bins, th, 'svL', 2);
