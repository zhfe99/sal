function wsSal = movSalL(src, wsSmp, wsFlow, wsAcc, wsSeg, parSal, varargin)
...
fold = sprintf('mov/%s/sal', src.tag);
...
if svL == 2 && exist(matpath, 'file')
    prInOut('movSalL', 'old, %s', prex);
    wsSal = matFld(matpath, 'wsSal');
    wsSal.vdo = vdopath;
    return;
end
prIn('movSalL', 'new, %s', prex);
hTi = tic;
...
% flow
hrFlow = movRIn(src, 'flow', wsFlow.parFlow);
