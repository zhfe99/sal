function wsVox = movVox(src, wsFlow, parVox, varargin)
...
fold = sprintf('mov/%s/vox', src.tag);
...
if svL == 2 && exist(matpath, 'file')
    prInOut('movVox', 'old, %s', prex);
    wsVox = matFld(matpath, 'wsVox');
    wsVox.vdo = vdopath;
    return;
end
prIn('movVox', 'new, %s', prex);
